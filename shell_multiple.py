...
variable bending stiffness (cf Fig 2ab).
...
mpiexec -n 4 python shell_multiple.py $SLURM_ARRAY_TASK_ID $SLURM_ARRAY_TASK_COUNT
________________
"""
...
#ratios = [0.005, 0.01, 0.05, 0.1, 0.5] # run 59
ratios = [0.005, 0.01, 0.05, 0.07, 0.1]
...
# assign work
abs_idx = np.arange(my_task_id-1, len(kappas), num_tasks)
# variable bending rigidity stiffness
my_parameters = kappas[my_task_id-1:len(kappas):num_tasks] 
...
for idx, parameter in enumerate(my_parameters):
    ...
    params = {'kappa':kappa, 'young':Young2d, 'kT':kT, 'p':pressure, 'R':R, 'Rc':10*R, 'poisson':0.3, 'visco':visco,
             'T0':100*minute, 'rampT':20*minute}
     
    output_filename = './run60/d3sh2f_Rc10_fvk1e4_{0}_run60_R25.mat'.format(abs_idx[idx])
    mid_filename = 'snapshots60_{0}'.format(abs_idx[idx])
    foldername = tmpdir
